...
$WebsiteSlug = "/admin_movie/movie-lien-ket";
if (GetParam("kw")) {
    $kw = sql_escape(GetParam("kw"));
    $LinkPage = $WebsiteSlug . "?kw=$kw&movie_id=$movie_id&p=";
    $SQL = "WHERE NAME LIKE '%$kw%' OR url LIKE '%$kw%'";
} else $LinkPage = $WebsiteSlug . "?movie_id=$movie_id&p=";
...
<head>
    <?php
    $title_admin = "Thêm Liên Kết Cho Phim {$Movie['name']}";
                    ...
                    <div class="row">
                        ...
                        <div class="card col-lg-12" id="NewItem">
                            <div class="card-body">
                                <form submit-ajax="ngockush" form-action="AddNewDatabase" action="<?= URL ?>/admin/server/api" method="POST" form-check="true">
                                    ...
                                    <input type="text" class="form-control" name="New[movie_id]" value="<?= $movie_id ?>" style="display: none;">
                        ...
                        <div class="card col-lg-12">
                            <div class="card-header">
                                <div class="row mb-3">
                                    <div class="col-6">
                                        <h4 class="card-title">Danh Sách Liên Kết Phim <?= $Movie['name'] ?></h4>
                                        <button class="btn btn-danger-gradien" type="button" onclick="MultiDel('<?= $table ?>');">Xóa Đã Chọn</button>
                                    </div>
                                    <div class="col-4">
                                        <input type="text" class="form-control" id="keyword" placeholder="Nhập Tên Liên Kết Cần Tìm">
                                    </div>
                                    <div class="col-2">
