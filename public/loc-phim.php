...
if ($value[2]) {
    $BASE64 = base64_decode(sql_escape($value[2]));
    $cate = RegexParam("[", "]", $BASE64, 2);
    $year = RegexParam("[", "]", $BASE64, 3);
    $LoaiPhim = RegexParam("[", "]", $BASE64, 4);
    $TrangThai = RegexParam("[", "]", $BASE64, 5);
    if ($cate) {
        $category = explode(",", $cate);
        foreach ($category as $key => $value) {
            $MySql .= " AND cate LIKE '%\"cate_id\":\"$value\"%'";
        }
    }
    if ($year) {
        $LocYear = explode(",", $year);
        foreach ($LocYear as $key => $value) {
            $MySql .= " AND year LIKE '%$value%'";
        }
    }
    if ($LoaiPhim) {
        $LoaiPhim = explode(",", $LoaiPhim);
        foreach ($LoaiPhim as $key => $value) {
            if ($value == 1) {
                $LoaiPhim = "Phim Lẻ";
            } else $LoaiPhim = "Phim Bộ";
            $MySql .= " AND loai_phim LIKE '%$LoaiPhim%'";
        }
    }
    if ($TrangThai) {
        $TrangThai = explode(",", $TrangThai);
        foreach ($TrangThai as $key => $value) {
            if ($value == 1) {
                $TrangThai = "Đang Cập Nhật";
            } else $TrangThai = "Hoàn Thành";
            $MySql .= " AND trang_thai LIKE '%$TrangThai%'";
        }
    }
} else $BASE64 = '[[],[],[],[]]';
?>
...
<head>
    <?php
    $title = "Trang Lọc Phim - {$cf['title']}";
    $description =  "Trang Lọc Phim - {$cf['title']}";
            ...
            <div id="filter-page">
                <div class="margin-10-0 bg-brown flex flex-space-auto">
                    <div class="fs-17 fw-700 padding-0-20 color-gray inline-flex height-40 flex-hozi-center bg-black border-l-t">
                        <h3 class="section-title"><span>Trang Lọc Phim</span></h3>
                    </div>
                </div>
                ...
                <div class="movies-list">
                    ...
                    $PAGE = CheckPages('movie', "WHERE public = 'true' $MySql", 30, $NumPage);
                    if ($PAGE['total'] >= 1) {
                        $arr = $mysql->query("SELECT * FROM " . DATABASE_FX . "movie WHERE public = 'true' $MySql ORDER BY id DESC LIMIT {$PAGE['start']},30");
                ...
                <?= view_pages($PAGE['total'], 30, $NumPage, URL . "/loc-phim/" . base64_encode($BASE64) . "?p=") ?>
                <script>
                    const $data_filter_raw = JSON.parse("<?= $BASE64 ?>");
                </script>
