...
<head>
    <?php
    $title = "Lịch Chiếu $NameDay {$cf['title']} - Trang $NumPage";
    $description =  "Lịch Chiếu $NameDay - {$cf['title']} - Trang $NumPage";
            ...
            <div id="filter-page">
                <div class="margin-10-0 bg-brown flex flex-space-auto">
                    <div class="fs-17 fw-700 padding-0-20 color-gray inline-flex height-40 flex-hozi-center bg-black border-l-t">
                        <h3 class="section-title"><span>Lịch Chiếu <?= $NameDay ?> Trang <?= $NumPage ?></span></h3>
                    </div>
                </div>
                <?php require_once(ROOT_DIR . '/view/filter.php'); ?>
                <div class="movies-list">
                    <?php
                    $PAGE = CheckPages('movie', "WHERE public = 'true' AND lich_chieu LIKE '%\"days\":\"$days\"%'", 15, $NumPage);
                    if ($PAGE['total'] >= 1) {
                        $arr = $mysql->query("SELECT * FROM " . DATABASE_FX . "movie WHERE public = 'true' AND lich_chieu LIKE '%\"days\":\"$days\"%' ORDER BY timestap DESC LIMIT {$PAGE['start']},15");
                ...
                <?= view_pages($PAGE['total'], 15, $NumPage, URL . "/lich-chieu.html?day=$days&p=") ?>
                <script>
